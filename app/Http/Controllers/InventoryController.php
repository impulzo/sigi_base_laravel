            ...
            foreach ($json as $item) {
                ...
                if ($movement->type == "E") {
                    ...
                    if ($item->quantity <= $product->stock) {
                        ...
                        $flag = false;
                        $data =  [
                            'message'    => "La cantidad de salida supera la cantidad actual del producto " . $product->product->name . " en la sucursal " . $product->sucursal->name,
                            'alert-type' => 'error',
                        ];
                        break;
                    }
                    ...
                    if($product->office_id == $item->office_id){
                        $flag = false;
                        $data =  [
                            'message'    => "La sucursal de traspaso debe ser diferente a la sucursal origen del producto " . $product->product->name . " en la sucursal " . $product->sucursal->name,
                            'alert-type' => 'error',
                        ];
                        break;
                    }else if ($item->quantity <= $product->stock) {
                        $product->stock -= $item->quantity;
                        $product_transfer = Inventory::where('product_id', '=', $product->product_id)->where('office_id', '=', $item->office_id)->first();
